...
OVERRIDES = {
    "5f8c4a86-5605-4c6d-8176-55ba3501bad4": {
        "agents": ["financier"],
        "chain": "Telegram, Smart-Lab, ДОХОДЪ и MOEX → скрипт Бобби → шаблонное уведомление → Алексей",
        "goal": "Не пропускать упоминания облигаций из портфелей Алексея.",
        "desc": "Детерминированный скрипт Бобби проверяет 11 Telegram-источников, два раздела Smart-Lab, блог ДОХОДЪ и новости Московской биржи, сохраняет новые материалы, ищет совпадения по ISIN, SECID, выпуску, эмитенту и ИНН и при наличии результата отправляет Алексею одно уведомление по фиксированному шаблону. Для каждого источника показывает ✅ при успешной проверке или 🚫 при ошибке. ИИ не используется.",
        "additional": "Cron активен каждые 15 минут и выполняется скриптом Бобби без ИИ.\n\n"
        "Источники:\n"
        "• 11 Telegram-источников: GoodBonds, oblig_flood, Bonds_Wizzard_Chat, philippovich_bonds, ludomanechka, cbonds, probonds, ivolgavdo, marythebond, oblig_news и finpokaz.\n"
        "• Smart-Lab: https://smart-lab.ru/allblog/ и https://smart-lab.ru/bonds/. Адрес /bond/ возвращает 404, поэтому используется действующий /bonds/.\n"
        "• Блог УК ДОХОДЪ: https://www.dohod.ru/blog.\n"
        "• Новости Московской биржи: https://www.moex.com/ru/news/ через официальный RSS всех новостей.\n"
        "• Всего контролируется 15 адресов.\n\n"
        "Защита от потерь и дублей:\n"
        "1. Для каждого потока хранится отдельный постоянный курсор или стабильный идентификатор URL.\n"
        "2. Каждый запуск дочитывает всё после курсора, включая данные после пропущенного цикла.\n"
        "3. Курсор продвигается только после успешного сохранения.\n"
        "4. Ошибочный диапазон повторно проверяется в следующем цикле.\n"
        "5. SQLite блокирует дубли по уникальному ключу «источник + ID».\n"
        "6. Smart-Lab bonds отслеживается по отдельным веткам: для каждой хранится счётчик комментариев и последний ID; при изменении счётчика скрипт постранично дочитывает ветку до сохранённого ID.\n"
        "7. Для Smart-Lab allblog и новостей MOEX сохраняются ID, дата, заголовок, URL и хэш; полный текст сохраняется при совпадении с портфелем.\n\n"
        "Идентификация эмитентов:\n"
        "• Для 53 уникальных бумаг хранится официальный справочник MOEX: ISIN, SECID, юридическое наименование эмитента и ИНН.\n"
        "• Полнота справочника проверяется при каждом запуске; неполная или некорректная запись останавливает цикл с явной ошибкой.\n"
        "• ИНН используется как точный признак совпадения и показывается в уведомлении вместе с юридическим наименованием эмитента.\n\n"
        "Доставка сигналов без ИИ:\n"
        "1. Скрипт каждые 15 минут собирает материалы.\n"
        "2. Скрипт сопоставляет их с позициями портфеля.\n"
        "3. При совпадении сам формирует сообщение по фиксированному шаблону.\n"
        "4. Обычное уведомление отправляется напрямую от Бобби через Telegram-аккаунт financier — без Продюсера и без запуска ИИ.\n"
        "5. В уведомлении указываются: точное или возможное совпадение, позиция и ISIN, юридический эмитент и ИНН, портфель, основание совпадения, источник, время, точная цитата, ссылка и статус проверки каждого из 15 адресов.\n"
        "6. Технические аварийные сообщения о падении cron приходят от Продюсера, чтобы не смешивать их с инвестиционными сигналами.\n\n"
        "Проверка: 15 адресов контролируются независимо. Smart-Lab bonds использует отдельные курсоры по веткам, MOEX — стабильный номер новости из официального RSS. При первом подключении MOEX и finpokaz доступная история сохранена без ретро-уведомлений; новые публикации проверяются с последующих циклов. В уведомлении каждый адрес получает ✅ или 🚫.",
    },
    ...
    "2d886b40-a26e-4890-806f-4c482eeaa509": {
        ...
        "desc": "Бобби получает цены MOEX по 53 ISIN и сравнивает их только с подтверждённой ценой покупки. Google Sheets читается с тремя сетевыми повторами через 3, 10 и 30 секунд; сетевые, авторизационные и содержательные ошибки различаются. Позиции без базы пропускаются; сообщение приходит лишь при отклонении от 2%.",
    },
    "d5907fa6-2de9-4789-abe1-af8233ecd327": {
        ...
        "desc": "Через 45 минут после каждого ценового цикла Бос проверяет его результат. Сетевой сбой вызывает ровно один повтор; ошибка авторизации или данных не перезапускается. Реальная ошибка, отсутствие или зависание запуска вызывает не более одного перезапуска; штатный цикл не создаёт сообщений.",
    },
...
YANA_IDS = {
    ...
    "8fc32d48-1677-4e5a-9024-2a8938d59767",
    "807225aa-55f5-420c-b525-d49bf4060e50",
    "cdff1be4-f4c7-40c9-a715-598d02b9fbd2",
}
...
PROCESS_STEPS = {
    ...
    "5f8c4a86-5605-4c6d-8176-55ba3501bad4": [
        ("Дочитать всё после сохранённого курсора в Telegram, Smart-Lab, блоге ДОХОДЪ и новостях MOEX", "Бобби"),
        ("Сохранить записи и обновить курсоры без пропусков и дублей", "Бобби"),
        ("Найти совпадения по ISIN, SECID, выпуску, эмитенту и ИНН", "Бобби"),
    ...
    "2d886b40-a26e-4890-806f-4c482eeaa509": [
        ...
        ("Прочитать Google Sheets с сетевыми повторами 3/10/30 секунд", "Бобби"),
    ...
    "d5907fa6-2de9-4789-abe1-af8233ecd327": [
        ...
        ("При сетевом сбое выполнить ровно один повтор", "Бос"),
        ("Не повторять ошибки авторизации или некорректных данных", "Бос"),
        ("При ошибке, отсутствии или зависании выполнить не более одного перезапуска", "Бос"),
...
PORTFOLIO_TG_RESOURCES = [
    ...
    ("Telegram — finpokaz", "https://t.me/finpokaz", "public"),
    ...
    ("MOEX — новости", "https://www.moex.com/ru/news/", "public"),
]
