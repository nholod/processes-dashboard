...
OVERRIDES = {
    "5f8c4a86-5605-4c6d-8176-55ba3501bad4": {
        ...
        "• Всего контролируется 13 адресов.\n\n"
        ...
        "6. Smart-Lab bonds отслеживается по 787 отдельным веткам: для каждой хранится счётчик комментариев и последний ID; при изменении счётчика скрипт постранично дочитывает ветку до сохранённого ID.\n"
        "7. Для Smart-Lab allblog сохраняются ID, дата, заголовок, URL и хэш; полный текст сохраняется при совпадении с портфелем.\n\n"
        ...
        "5. В уведомлении указываются: точное или возможное совпадение, позиция и ISIN, портфель, основание совпадения, источник, время, точная цитата, ссылка и статус проверки каждого из 13 адресов.\n"
        "6. Технические аварийные сообщения о падении cron приходят от Продюсера, чтобы не смешивать их с инвестиционными сигналами.\n\n"
        "Проверка: все 13 адресов доступны; после перехода Smart-Lab bonds на 787 отдельных курсоров контрольный полный запуск завершён со статусом ok и обработал новые комментарии из изменившихся веток. При первоначальной проверке 11 источников за 15 минут было найдено 17 новых материалов и 0 совпадений с портфелем; после добавления oblig_news и блога ДОХОДЪ новых совпадений также не обнаружено. В уведомлении каждый адрес получает ✅ или 🚫.",
    },
...
PROCESS_STEPS = {
    ...
    "5f8c4a86-5605-4c6d-8176-55ba3501bad4": [
        ("Дочитать всё после сохранённого курсора в Telegram, Smart-Lab и блоге ДОХОДЪ", "Бобби"),
